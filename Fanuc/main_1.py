            ...
            while 1 :
                ...
                if  (bit_status(int(conn.readpmc(1,1,0,1)[0]), 6) == 0) and (data_logged == 0) and (int(conn.readpmc(1,5,32,1)[32]) == 1) :
                    ...
                    try:
                        ...
                        M_DF.to_csv(f"Report_{date.today()}.csv", mode='a', header=not os.path.exists(f"Report_{date.today()}.csv"))
